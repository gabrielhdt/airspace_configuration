...
\begin{abstract}
  Dynamic airspace configuration falls under highly combinatorial partitioning
  problems. Since exact methods tend to be overwhelmed by the complexity of such
  problems, a stochastic approach is here considered. This paper presents the
  the Monte Carlo tree search algorithm using UCT search and adapted to one
  player games. The application of the algorithm to the airspace partitioning is
...
Currently, configuration is mainly decided on the fly by chief officer.
...
step \(i\). Thoses lists has been randomly generated. The set of lists form a
scenario which is used in all following tests.
...
\begin{figure}
  \begin{tikzpicture}
    \begin{axis}
      ...
      \addplot+[dashed, domain=0:2] {1268};
    \end{axis}
  \end{tikzpicture}
  \caption{}\label{fig:shortterm}
\end{figure}
