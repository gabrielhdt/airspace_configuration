...
\begin{equation}
  \text{UCB} = \mu_{v,a} + 2 \beta \sqrt{\frac{2 \log T_v}{T_{v,a}}}
\end{equation}

\paragraph{Expansion}
The above step is carried out only if, considering a node \(F\), a simulation
has been run through each child available from \(F\). In the case where one or
