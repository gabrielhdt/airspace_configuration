...
algorithm~\ref{alg:backpropagate-tuned} which introduces the value \(m_v\)
associated to a node \(v\) to compute the standard deviation via the formula
\begin{equation}
  \sigma^2_v = \frac{m_v}{T_v}.
\end{equation}
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \State{}\(\mu_v \gets \mu_v + \frac{\delta}{T_v + 1}\)
    \State{}\(\delta' \gets r - \mu_v\)
    \State{}\(m_v \gets m_v + \delta \delta'\)
    \State{}\(T_v \gets T_v + 1\)
    \EndProcedure{}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \EndFunction{}
    \Function{NextNode}{$u$}
    \State{}\Return{$\argmax\{\mu_{u,a} | a \in \mathcal{A}(v)\}$}
    \EndFunction{}
  \end{algorithmic}
\end{algorithm}
\subsection{A\(^*\)}
...
follow : $\alpha > \lambda > \beta > \gamma$.
