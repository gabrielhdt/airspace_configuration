...
the left hand term increases the value of less explored nodes to consider them
...
In this paper, we consider a high branching factor problem.

Let \(M\) be the number of edges between the inital state and a final state and
\(K\) the branching factor.
Then, the complexity of the A\(^*\) is expected to be, in the worst case,
exponential in \(M\) (see~\cite{alliotschiex2002ia&it}).
...
payoff is \(O\left(\frac{\log n}{n}\right)\). Furthermore, the failure
probability\footnote{probability of not selecting the best child} converges to
zero at a polynomial rate.
