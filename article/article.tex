...
\begin{itemize}
  ...
  \item $\mu_{v, a}$: mean of all reward comming from node $v$ with action $a$
  \item $T_v$: number of simulation where the final path include the node $v$
  \item $T_{v, a}$: number of simulation where the final path include the
    node $F$ with action $a$
  \item \(\mathcal{A}(v)\) the available actions from a node \(v\)
  \item \(p\colon \mathcal{S} \times \mathcal{A} \to \mathcal{S}\) the
    production function, giving the resulting state of action \(a\) applied on a
    state \(s\).
\end{itemize}
...
\paragraph{Selection~\ref{alg:tree_pol}}
...
\begin{equation}
  \text{UCB1} = \mu_{v,a} + 2 \beta \sqrt{\frac{2 \log T_v}{T_{v,a}}}
\end{equation}
...
alternative using the standard deviation is proposed in~\cite{sebag2010fuse}
called the \emph{UCB1-tuned} equation
\begin{equation}
  \mu_{v,a} +
  \sqrt{%
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \Function{TreePolicy}{path of nodes $\pi$}
    \State{} $v \gets$ head of $\pi$
    \State{} \Call{Deploy}{$v$}
    \If{$v$ is terminal}
    \State{}\Return{$\pi$}
    \Else{}
    \If{$v$ is not fully expanded}
    \State{}\Return{\Call{Expand}{$v$} $\cons \pi$}
    \Else{}
    \State $f \gets$ \Call{BestChild}{$v$}
    \State{}\Return{%
      \Call{TreePolicy}{$f \cons \pi$}
    }
    \EndIf{}
    \EndIf{}
    \EndFunction{}
  ...
  \begin{algorithmic}
    ...
    \State{} \Return{$\argmax\{\Call{UCB1-tuned}{v'} |
    v' \text{\,children of\,} v\}$}
    \EndFunction{}
  \end{algorithmic}
\end{algorithm}

\paragraph{Expansion~\ref{alg:expansion}}
...
\begin{algorithm}
  \caption{%
    Expansion (\(p\colon \mathcal{S} \times \mathcal{A} \to \mathcal{S}\)
    is the transition function)
  }\label{alg:expansion}
  \begin{algorithmic}
    \Function{Expand}{node $v$}
    \State{} $\ell \gets \{p(v, a) | a\in\mathcal{A}(v), T_{v,a} = 0\}$
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \Procedure{Backpropagate}{path $\pi$, $r$}
    \State{} \(\delta \gets\ r - \mu_v\)
    \State{} \(\mu_v \gets \mu_v + \frac{\delta}{T_v + 1}\)
    \State{} \(T_v \gets T_v + 1\)
    \EndProcedure{}
  \end{algorithmic}
\end{algorithm}

For the UCB1-tuned, the standard deviation has to be computet. It results in
algorithm~\ref{alg:backpropagate-tuned}
\begin{algorithm}
