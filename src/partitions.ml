...
type context= {
    gmap: Sset.t Smap.t; (* key= group ID -> data= set of elements *)
    (* [gmap] stores for each group the elements it is made of. *)

    emap: Sset.t Smap.t (* key = element -> data= groups containing element *)
    (* [emap] stores for each element the groups it belongs to.*)
  };;

let member gmap e g =
  try Sset.mem e (Smap.find g gmap)
...
let possible_groups ctx s =
  ...
  Sset.fold
    (fun e acc -> Sset.inter (Smap.find e ctx.emap) acc)
    s init;;
...
let update_compatible_groups ctx e others =
  ...
  List.map
    (fun (eset,glist) ->
      let new_glist=
        List.filter (fun g -> not (member ctx.gmap e g)) glist in
...
(*                Printf.printf "Partition length= %d\n" (List.length x); *)
	        ...
	        if check x then update x acc (*x::acc*) else acc
              with Not_valid -> acc in
        add ((elts, glist)::revleft) rest new_acc
    | [] ->
        try
          let elts= Sset.singleton e in
          let all_others =
            List.fold_left (fun acc (eset,_)-> Sset.union eset acc)
              Sset.empty (List.rev revleft) in
          let glist=
            List.filter
              (fun g->
                let eset= Smap.find g ctx.gmap in
                Sset.is_empty (Sset.inter eset all_others))
              (Sset.elements (Smap.find e ctx.emap)) in
          match glist with
            [] -> acc
          | _->
              let new_revleft= update_compatible_groups ctx e revleft in
...
(*              Printf.printf "New partition length= %d\n" (List.length x); *)
                flush stdout;
              if check x then update x acc (*x::acc*) else acc
        with Not_found | Not_valid -> acc in
  add [] partition acc;;
...
let partitions ctx check_node le =
  ...
  let rec loop le partition acc =
	...
	List.fold_left
	  (fun acc child -> loop es child acc)
	  acc children
    | [] -> acc in
  loop le [] [];;
...
let count ctx check_node le =
  ...
  let elements = set_of_list le in
  let ctx= update_context ctx elements in
  let update = fun _x acc -> acc+1 in
  let rec loop le partition acc =
    match le with
      [e] -> add_element ctx check_node e partition update acc
    | e::es ->
	let children=
          add_element ctx check_node e partition (fun x acc ->x::acc) [] in
	List.fold_left
	  (fun acc child -> loop es child acc)
	  acc children
    | [] -> acc in
...
let recombine2 ctx nmax s1 s2 =
  ...
  let rec changed partition =
    match partition with
      (s,gl)::rest -> not (Sset.equal s s1 || Sset.equal s s2) && changed rest
    | [] -> true in
  let check =
    match nmax with
...
let merge ctx s1 s2 =
  ...
  try
    ...
    if Sset.is_empty gset then []
    else [[(s,Sset.elements gset)]]
  with Not_found -> [] (* No compatible groups *)
;;
      ...
      (fun acc sub_partition ->
        let new_partition=
          List.fold_left (fun acc x -> x::acc)
            (List.rev_append revleft rest) sub_partition in
        new_partition::acc)
      acc sub_partitions in
        ...
        let rec inner_loop revleft2 right2 acc =
          match right2 with
            (s2,gl2)::rest2 ->
              ...
              let new_acc=
                add revleft2 rest2 transfers
                  (add revleft2 rest2 merges acc) in
              inner_loop ((s2,gl2)::revleft2) rest2 new_acc
          | [] -> outer_loop ((s1,gl1)::revleft1) rest1 acc in
...
let le= ["1";"2";"3";"4";"5"];;
let lp= partitions ctx (fun _-> true) le;;
print_partitions lp

print_map ctx.gmap;;
print_map ctx.emap;;

let lp= partitions ctx (fun l-> List.length l = 3) le;;

print_partitions lp;;

let le2= ["3";"4";"5"];;
let ctx2= update_context ctx (set_of_list le2);;

print_map ctx2.gmap;;
print_map ctx2.emap;;

let lp2= partitions ctx (fun _-> true) le2;;

print_partitions lp2;;

let s1= set_of_list ["1";"2";"3"];;
let s2= set_of_list ["4";"5"];;



let s1= set_of_list ["1";"5"];;
let s2= set_of_list ["2";"3"];;

let s1= set_of_list [];;
let s2= set_of_list ["1";"2";"3"];;

let s1= set_of_list ["1";"2";"3"];;
let s2= set_of_list ["4";"5"];;


let r= recombine2 ctx None s1 s2;;
let r= recombine2 ctx (Some 3) s1 s2;;

print_partitions r;;

let s1= set_of_list ["1"];;
let s2= set_of_list ["2";"3"];;

print_partitions (merge ctx s1 s2);;
print_partitions (transfer ctx s1 s2);;

let s= Sset.union s1 s2;;
Printf.printf "%s\n" (set2string s);;
let gset= possible_groups ctx s;;
Printf.printf "%s\n" (set2string gset);;

let ctx3= update_context ctx s;;
print_map ctx3.gmap;;
print_map ctx3.emap;;

print_partitions (split2 ctx s);;

let s123= set_of_list ["1";"2";"3"];;
let s4= set_of_list ["4"];;
let s5= set_of_list ["5"];;

let cg123= compatible_groups ctx s123 (Sset.union s4 s5);;
let cg4= compatible_groups ctx s4 (Sset.union s123 s5);;
let cg5= compatible_groups ctx s5 (Sset.union s123 s4);;

let p= [(s123,cg123);(s4,cg4);(s5,cg5)];;
let lp= recombine ctx p;;
print_partitions lp;;

let p2= [(s4,cg4);(s5,cg5);(s123,cg123)];;
let lp2= recombine ctx p2;;
print_partitions lp2;;
*)
